...
| Build, lint, typecheck, evaluation integrity, and tests pass | `pnpm check` runs Oxlint with warnings denied, strict TypeScript, `pnpm run eval -- check` over strict scenario/assessor schemas and frozen fixture SHA-256 digests, the tsdown Host and browser builds, and the full Vitest inventory below. The build runs before browser-artifact tests. |
| Paired evaluation isolates Raven | `scripts/live-evaluation.ts` generates two isolated profiles from one exact PTC include and one shared absolute staging cwd; the treatment preset adds only Raven's sibling row. It records order, environment/version cleanliness, scenario/input/source/base hashes, equal initial workspace hashes, raw Session JSONL, complete `llm/stream` call ledgers, prompts, tool schemas, artifacts, and evidence-tree hashes. `steering-checkpoint` crosses a real process boundary through JSONL persistence. `.github/workflows/evaluation.yml` is manual/paid and archives raw evidence; release CI runs the keyless paired/restart smoke before publish. |
| Durable output as a valid llm-wiki | `tests/unit/wiki.test.ts` exports a completed Task as llm-wiki bytes: a Task-addressed artifact page; Task-and-resource-addressed immutable `raw/` pages whose `sha256` covers exactly their bodies; absent preconditions; an idempotent log marker; and SCHEMA/index/log seeds only under `init`. Same-Task export is byte-identical and two Tasks with the same title and Task-local Source IDs cannot overwrite one another. Raven emits bytes and never writes files, so the repository stays readable by the llm-wiki skill and Obsidian. |
...
| Selected-Skeleton multi-model drafting that cannot become evidence | `tests/unit/drafting.test.ts`, `tests/integration/drafting.test.ts`, and `tests/integration/structure-studio.test.ts` prove independent section candidates, all eight adversarial criteria, intact required refinement context, comparison before synthesis, rejection of winner-copy output without two route-specific contributions, delimiter-safe untrusted rendering, visible truncation, exact selected-section/purpose/Claim/Insight/evidence-gap/counterargument context, and bounded durable provenance without generated text. `research | synthesis | structure` recovery survives replay/status/context and blocks prose publication until a later draft. Route subset/refusal, stalled-stream deadlines, partial and total failure, legacy DraftGenerator compatibility, single-model/main-agent fallback, and successful fallback/selected-Skeleton Completion remain covered. Even synthesized wording creates no Source or Claim and cannot satisfy a grounding-required Completion without verified evidence. |
...
- `tests/unit/drafting.test.ts`
  - independent candidate prompts with no sibling prose leakage;
  - complete eight-criterion adversarial comparison;
  - intact long refinement contracts and structured contributions from multiple successful routes;
  - winner-copy rejection, visible truncation, and delimiter-safe untrusted rendering;
  - research-gap short-circuit before prose synthesis;
  - explicit single-model fallback after a route failure.
...
- `tests/integration/drafting.test.ts`
  ...
  - the real Harness `ctx.llm` adapter executing independent candidate, critique, and synthesis calls on exact routes;
  - every configured route drafted, each variant and synthesized result laid out one sentence per line;
  - a requested subset honoured and an unconfigured route refused with the configured set named;
  - no-route and all-failed paths reported explicitly, with successful main-agent fallback Completion;
  - surviving single-model material kept when one route fails, without pretending comparison occurred;
  - bounded route/path provenance persisted and replayed, retaining no candidate, critique, or synthesis text;
  - variants and synthesized prose never reaching the evidence floor;
...
- `tests/integration/structure-studio.test.ts`
  ...
  - exact selected-section drafting with full Claim/Insight records, verified evidence excerpts, purpose, evidence needs, counterarguments, audience/constraints, and wider Skeleton flow;
  - multi-model comparison/synthesis provenance, transient generated text, successful post-synthesis Raven Completion, and Structure Studio recovery after an adversarial architecture defect;
